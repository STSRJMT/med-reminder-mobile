...
function toLocalDT(date) {
  ...
  return { mysqlDT: `${y}-${mo}-${d} ${h}:${mi}:${s}`, dateStr: `${y}-${mo}-${d}` };
}

function toDowToken(dateStr) {
  return String(new Date(dateStr + "T00:00:00").getDay());
}
...
router.post("/schedules", auth(["elderly"]), async (req, res) => {
  try {
    ...
    if (!name || !timeHHMM) return res.status(400).json({ message: "ข้อมูลไม่ครบ" });
  ...
  try {
    ...
    const [rows] = await db.query(
      `SELECT s.id, m.id AS medication_id, m.elderly_user_id
       FROM schedules s JOIN medications m ON m.id = s.medication_id WHERE s.id = ?`, [id]
    );
    if (rows.length === 0) return res.status(404).json({ message: "ไม่พบข้อมูล" });
    if (rows[0].elderly_user_id !== req.user.id) return res.status(403).json({ message: "ไม่มีสิทธิ์" });
    await db.query(
...
router.delete("/schedules/:id", auth(["elderly"]), async (req, res) => {
  try {
    const { id } = req.params;
    const [rows] = await db.query(
      `SELECT s.id, m.elderly_user_id FROM schedules s JOIN medications m ON m.id = s.medication_id WHERE s.id = ?`, [id]
    );
    if (rows.length === 0) return res.status(404).json({ message: "ไม่พบข้อมูล" });
    if (rows[0].elderly_user_id !== req.user.id) return res.status(403).json({ message: "ไม่มีสิทธิ์" });
...
// ✅ GET /elderly/today?date=YYYY-MM-DD — เพิ่ม takenAt
router.get("/today", auth(["elderly"]), async (req, res) => {
  ...
  const [rows] = await db.query(
    `SELECT s.id AS scheduleId, s.time_hhmm AS timeHHMM, s.days_of_week AS daysOfWeek,
            ...
            (SELECT il.taken_at FROM intake_logs il
             WHERE il.schedule_id = s.id AND il.elderly_user_id = ?
               AND DATE(il.taken_at) = ? ORDER BY il.taken_at DESC LIMIT 1) AS takenAt
     ...
     ORDER BY s.time_hhmm ASC`,
    [req.user.id, date, req.user.id, date, req.user.id, date, dow]
  );
  res.json({ date, dow, items: rows });
});

// ✅ POST /elderly/intake — รับ takenAtISO + auto-detect ล่าช้า
  ...
  try {
    const [scheduleInfo] = await db.query(
      `SELECT m.name AS medication_name, u.name AS elderly_name
       FROM schedules s JOIN medications m ON m.id = s.medication_id
       JOIN users u ON u.id = m.elderly_user_id WHERE s.id = ?`, [scheduleId]
    );
    const [caregivers] = await db.query(
      `SELECT u.fcm_token FROM users u JOIN care_relationships cr ON cr.caregiver_user_id = u.id
       WHERE cr.elderly_user_id = ? AND u.fcm_token IS NOT NULL`, [req.user.id]
    );
    const medName     = scheduleInfo[0]?.medication_name ?? "ยา";
    const elderlyName = scheduleInfo[0]?.elderly_name ?? "ผู้สูงอายุ";
    const statusLabel = status === "taken" ? "กินยาแล้ว ✓" : status === "late" ? "กินยาล่าช้า ⏰" : "ข้ามมื้อยา ✗";
    for (const cg of caregivers) {
      if (!cg.fcm_token) continue;
      await sendExpoPush(cg.fcm_token, `${elderlyName} — ${statusLabel}`, `ยา "${medName}"`);
    }
  ...
  try {
    ...
    const [schedules] = await db.query(
      `SELECT s.id, s.time_hhmm, s.days_of_week, s.start_date
       FROM schedules s JOIN medications m ON m.id = s.medication_id
       WHERE m.elderly_user_id = ?`, [elderlyId]
    );
    ...
    for (const s of schedules) {
      ...
      if (currentMinutes < h * 60 + m + MISSED_GRACE_MINUTES) continue;
      ...
      await db.query(
        `INSERT INTO intake_logs (schedule_id, elderly_user_id, status, taken_at) VALUES (?, ?, 'missed', ?)`,
...
router.get("/report", auth(["elderly"]), async (req, res) => {
  try {
    ...
    if (period === "day") {
      ...
      startDate = new Date(now); startDate.setDate(now.getDate() + diffToMonday); startDate.setHours(0,0,0,0);
    ...
    function countActiveDays(schedule, start, end) {
      ...
      const scheduleStart = schedule.start_date
        ...
        : start;
      let count = 0;
      const cursor = new Date(scheduleStart); cursor.setHours(0,0,0,0);
    ...
    const [schedules] = await db.query(
      `SELECT s.id, s.days_of_week, s.start_date, m.id AS med_id, m.name AS med_name
       FROM schedules s JOIN medications m ON m.id = s.medication_id
       WHERE m.elderly_user_id = ? AND (s.start_date IS NULL OR DATE(s.start_date) <= DATE(?))`,
      [elderlyId, endDate]
    );

    const [logs] = await db.query(
      `SELECT schedule_id, status FROM intake_logs WHERE elderly_user_id = ? AND taken_at BETWEEN ? AND ?`,
    ...
    const drugs = [...medMap.values()].map(m => ({
      name: m.name, taken: m.taken, totalFull: m.totalFull, totalSoFar: m.totalSoFar,
...
router.get("/:elderlyId", auth, async (req, res) => {
  try {
    const { elderlyId } = req.params;
    const [rows] = await db.query(
      "SELECT id, name, phone, age, address, disease FROM elderly WHERE id = ?", [elderlyId]
    );
